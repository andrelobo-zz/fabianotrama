import {
  ...
  valueFromASTUntyped,
  isDirective,
  getDirectiveValues as getDirectiveValues$1,
        ...
        if (schemaDirective) {
          ...
          if (schemaDirective.isRepeatable) {
            if (result[schemaDirective.name]) {
              result[schemaDirective.name] = result[
...
function printSchemaWithDirectives(schema, _options = {}) {
  ...
  for (const typeName in typesMap) {
    ...
    result.push(
      print(
        (_a = correctType(typeName, typesMap)) === null || _a === void 0
          ? void 0
          : _a.astNode
      )
    );
...
function correctType(typeName, typesMap) {
  ...
  const originalAstNode =
    type === null || type === void 0 ? void 0 : type.astNode;
  if (originalAstNode) {
    fixedAstNode.directives =
      originalAstNode === null || originalAstNode === void 0
        ? void 0
        : originalAstNode.directives;
    if (
      fixedAstNode &&
      "fields" in fixedAstNode &&
      originalAstNode &&
      "fields" in originalAstNode
    ) {
      for (const fieldDefinitionNode of fixedAstNode.fields) {
        const originalFieldDefinitionNode = originalAstNode.fields.find(
          field => field.name.value === fieldDefinitionNode.name.value
        );
        fieldDefinitionNode.directives =
          originalFieldDefinitionNode === null ||
          originalFieldDefinitionNode === void 0
            ? void 0
            : originalFieldDefinitionNode.directives;
        if (
          fieldDefinitionNode &&
          ...
          for (const argument of fieldDefinitionNode.arguments) {
            const originalArgumentNode =
              (_a = originalFieldDefinitionNode.arguments) === null ||
              _a === void 0
                ? void 0
                : _a.find(arg => arg.name.value === argument.name.value);
      ...
      fixedAstNode &&
      "values" in fixedAstNode &&
      originalAstNode &&
      "values" in originalAstNode
    ) {
      for (const valueDefinitionNode of fixedAstNode.values) {
        const originalValueDefinitionNode = originalAstNode.values.find(
          valueNode => valueNode.name.value === valueDefinitionNode.name.value
        );
        valueDefinitionNode.directives =
          originalValueDefinitionNode === null ||
          originalValueDefinitionNode === void 0
            ? void 0
            : originalValueDefinitionNode.directives;
      }
    }
  }
...
function getSchemaDefinition(schema) {
  if (
    !Object.getOwnPropertyDescriptor(schema, "astNode").get &&
    schema.astNode
  ) {
...
function resolveField({
  ...
  if (
    fieldTypeMap.has(fieldName) &&
    fieldTypeMap.get(fieldName) !== field.type.toString()
  ) {
...
function visitOutputType(visitedTypes, pruningContext, type) {
  ...
  if (isObjectType(type) || isInterfaceType(type)) {
    ...
    if (isInterfaceType(type)) {
      Object.keys(pruningContext.implementations[type.name]).forEach(
        typeName => {
          visitOutputType(
            visitedTypes,
            pruningContext,
...
function concatInlineFragments(type, fragments) {
  const fragmentSelections = fragments.reduce(
    (selections, fragment) =>
      selections.concat(fragment.selectionSet.selections),
    []
  );
  const deduplicatedFragmentSelection = deduplicateSelection(
    fragmentSelections
  );
  return {
    kind: Kind.INLINE_FRAGMENT,
...
function shouldIncludeNode(exeContext, node) {
  const skip = getDirectiveValues$1(
    GraphQLSkipDirective,
    node,
...
function visitRoot(
  ...
  const collectedFields = collectFields$1(
    exeContext,
    operationRootType,
...
function visitObjectValue(
  ...
  if (errors != null) {
    ...
    errorInfo.unpathedErrors = errorInfo.unpathedErrors.concat(
...
export {
  ...
  cloneSchema,
  cloneType,
  collectFields$1 as collectFields,
  compareNodes,
  compareStrings,
