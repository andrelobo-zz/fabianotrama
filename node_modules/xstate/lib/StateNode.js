...
var StateNode = /** @class */ (function() {
    ...
    this.invoke = utils_1
      ...
      .map(function(invokeConfig, i) {
        ...
        if (utils_1.isMachine(invokeConfig)) {
          ...
          utils_1.isMachine(invokeConfig.src) ||
          utils_1.isFunction(invokeConfig.src)
        ) {
          ...
          return invokeUtils_1.toInvokeDefinition(
            __assign(__assign({ id: invokeSrc }, invokeConfig), {
              src: invokeSrc
            })
          );
        } else {
          ...
          return invokeUtils_1.toInvokeDefinition(
            __assign(__assign({ id: invokeSource.type }, invokeConfig), {
              src: invokeSource
            })
          );
        }
  ...
  StateNode.prototype.getActions = function(
    ...
    var doneEvents = utils_1.flatten(
      transition.entrySet.map(function(sn) {
        ...
        events.push(
          actions_1.done(sn.id, sn.doneData), // TODO: deprecate - final states should not emit done events for their own state.
          actions_1.done(
            parent.id,
            sn.doneData
  ...
  StateNode.prototype.resolveTransition = function(
    ...
    var children = invokeActions.reduce(
      function(acc, action) {
        acc[action.activity.id] = Actor_1.createInvocableActor(
          action.activity,
          _this.machine,
          updatedContext,
          _event
            ...
            if (!environment_1.IS_PRODUCTION && key === NULL_EVENT) {
              utils_1.warn(
                ...
                "Empty string transition configs (e.g., `{ on: { '': ... }}`) for transient transitions are deprecated. Specify the transition in the `{ always: ... }` property instead. " +
                  ('Please check the `on` configuration for "#' +
                    _this.id +
                    '".')
              );
            }
            var transitionConfigArray = utils_1.toTransitionConfigArray(
              key,
